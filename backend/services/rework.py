"""GenPal orchestration helpers for duplicate detection and repair.

Thin glue over focused service modules. The terminal states for the global
duplicate repair are re-exported from constants for convenience.
"""
...
GENPAL_COLUMNS = constants.GENPAL_COLUMNS
FINAL_DUPLICATE_CHECK_PASSED = constants.FINAL_DUPLICATE_CHECK_PASSED
FINAL_MANUAL_REVIEW_REQUIRED = constants.FINAL_MANUAL_REVIEW_REQUIRED
FINAL_REPAIR_FAILED = constants.FINAL_REPAIR_FAILED
...
def resolve_internal_duplicates(
    ...
    *,
    ...
    findings = duplicate_detector.find_duplicates(rows, threshold, client=client, cost_sink=cost_sink)
    attempt = 0
        ...
        for pass_no in range(max_passes):
            ...
            for rn, canonical, members in candidates:
                ...
                ok = generator.rework_duplicate_row(
                    row, canonical_row, cluster_rows, nearby, client=client, cost_sink=cost_sink
                )
